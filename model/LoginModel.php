...
class LoginModel
{
    ...
    public function comprobarTipoUsuario($nombreUsuario, $clave){
    //compruebo si el usuario existe. si me devuelve un 1 es un admin si 0 un usuario comun
    $cero = "0";
    $uno = "1";

    $consultaPorUsuarioComun="SELECT * FROM Usuario WHERE nombreDeUsuario= '" .$nombreUsuario."'AND contrasenia= '" 
    . $clave ."'AND tipoDeUsuario = '".$cero."'";

    $consultaPorAdmin = "SELECT * FROM Usuario WHERE nombreDeUsuario= '" .$nombreUsuario."'AND contrasenia= '" 
    . $clave ."'AND tipoDeUsuario = '".$uno."'";
